column_data = sheet.col_values(1)  # Assuming you want to analyze the first column
mean = pd.Series(column_data).astype(float).mean()
median = pd.Series(column_data).astype(float).median()
std_dev = pd.Series(column_data).astype(float).std()
